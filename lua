...
RunService.RenderStepped:Connect(function()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        ...
        and plr.Character:FindFirstChild("HumanoidRootPart") then
            ...
            -- วาดเฉพาะคนที่อยู่ในจอ
            if onScreen then
                local line = Drawing.new("Line")
                line.Color = Color3.fromRGB(0, 180, 255) -- ฟ้า
                line.Thickness = 2
                line.Transparency = 1
...
ESPTab:Toggle({
    Title = "ESP Item",
    Description = "Show backpack & held items",
    Default = false,
    Callback = function(v)
        ESP_Item = v
        if not v then
            for _, gui in pairs(ItemLabels) do
                gui:Destroy()
            end
...
RunService.RenderStepped:Connect(function()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        ...
        and plr.Character:FindFirstChild("HumanoidRootPart") then
            ...
            local label = ItemLabels[plr]:FindFirstChildOfClass("TextLabel")

            if #items > 0 then
                label.Text = table.concat(items, "\n") -- 1 ชิ้น / 1 บรรทัด
            else
                label.Text = "No Item"
            end

            ItemLabels[plr].Enabled = true
        end
    end

    -- ลบ ESP คนออกเกม
    for plr, gui in pairs(ItemLabels) do
